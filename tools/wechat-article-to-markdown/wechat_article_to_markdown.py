...
async def download_all_images(
    ...
    print(f"Downloading {len(img_urls)} images (concurrency {IMAGE_CONCURRENCY})...")
    ...
    print(f"  [OK] {downloaded}/{len(img_urls)}")
    return url_map
...
def build_markdown(meta: dict, body_md: str) -> str:
    ...
    if meta.get("cover_image"):
        lines.append(f"cover_image: '{meta['cover_image']}'")
    lines.append("---")
    lines.append("")
...
async def fetch_article(url: str) -> None:
    ...
    if not meta["title"]:
        print("Error: Could not extract article title, possibly captcha.")
        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        (OUTPUT_DIR / "debug.html").write_text(html, encoding="utf-8")
        print("Saved raw HTML to output/debug.html")
        sys.exit(1)
    ...
    print(f"Markdown approx {len(md)} chars")


def main():
    if len(sys.argv) < 2:
        print("Usage: python wechat_article_to_markdown.py <wechat-article-url>")
        sys.exit(1)

    url = sys.argv[1]
    if not url.startswith("https://mp.weixin.qq.com/"):
        print("Error: Invalid WeChat URL")
        sys.exit(1)
